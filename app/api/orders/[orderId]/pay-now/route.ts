...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      // Log the line items to aid debugging if Stripe rejects them
      try {
        console.debug("Stripe checkout line items:", JSON.stringify(lineItems, null, 2));
      } catch (logError) {
        console.debug("Stripe line items (stringify failed)", logError);
      }
      ...
      return NextResponse.json(
        {
          ...
          // Expose the underlying Stripe/error message during development to aid debugging
          details: process.env.NODE_ENV === "development" ? checkoutErrorMessage : undefined,
        },
        { status: 500 }
      );
    }
